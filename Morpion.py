...
def tour(grille, joueur):
    ...
    colonne=input('Je te le met sur quel colonne? Sur : ')
    ligne=input('Tu le veux sur quel ligne? Sur : ')
    print("Du coup il sera la ("+colonne+","+ligne+")")
    while grille[int(colonne)+int(ligne)*3]!=" ":
        afficher_grille(grille)
        print("Mince ! C'est trop tard elle est deja joué :)")
        colonne = input('Je te le met sur quel colonne? Sur : ')
        ligne = input('Tu le veux sur quel ligne? Sur : ')
        print("Du coup il sera la ("+colonne+","+ligne+")")
